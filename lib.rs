...
const PROB_IMMORTAL:u8 = 2; 
const PROB_MYTHIC:u8 = 8; 
const PROB_LEGENDARY:u8 = 16;
const PROB_EPIC:u8 = 32;
...
impl Contract {
    ...
    pub fn checkin(&mut self, username: String, request: String) -> u8 {
        ...
        // Decide what to transfer for the player (default to common)
        let rand = self.toss();
        let mut result = 0;
        let mut token_id: TokenId = "0".to_string();
        
        if rand < PROB_IMMORTAL {            
            token_id = "6".to_string();
            result = 6;
        } else if (rand < PROB_MYTHIC) {
            token_id = "5".to_string();
            result = 5;
        } else if (rand < PROB_LEGENDARY) {
            token_id = "4".to_string();
            result = 4;
        } else if (rand < PROB_EPIC) {
            token_id = "3".to_string();
            result = 3;
        } else if (rand < PROB_RARE) {
            token_id = "2".to_string();
            result = 2;
        } else if (rand < PROB_UNCOMMON) {
        ...
        let reward = RewardData {
            reward: result,
            timestamp: timestamp,
            token_id: token_id_with_timestamp.clone(),
        };
        
        // Update reward
        let mut balance = self.rewards.get(&username).unwrap_or(vec![]);
        balance.push(reward);
        self.rewards.insert(&username, &balance);
        log!("Checkin successful! User: {}, Token: {}", username, token_id_with_timestamp.clone());                                
        ...
        let mut token_metadata = TokenMetadata {
            ...
            description: Some("Common NFT vSelf reward token".to_string()),
            ...
            starts_at: None,
            updated_at: None,
            extra: Some(request.clone()),
            reference: None,
            reference_hash: None,
        };
        
        if result == 1 {
            token_metadata.title = Some("Uncommon".to_string());
            token_metadata.description = Some("Uncommon NFT vSelf reward token".to_string());
        } else if result == 2 {
            token_metadata.title = Some("Rare".to_string());
            token_metadata.description = Some("Rare NFT vSelf reward token".to_string());            
        } else if result == 3 {
            token_metadata.title = Some("Epic".to_string());
            token_metadata.description = Some("Epic NFT vSelf reward token".to_string());            
        } else if result == 4 {
            token_metadata.title = Some("Legendary".to_string());
            token_metadata.description = Some("Legendary NFT vSelf reward token".to_string());            
        } else if result == 5 {
            token_metadata.title = Some("Mythic".to_string());
            token_metadata.description = Some("Mythic NFT vSelf reward token".to_string());            
        } else if result == 6 {
            token_metadata.title = Some("Immortal".to_string());
            token_metadata.description = Some("Immortal NFT vSelf reward token".to_string());            
        };              

        log!("Success! Minting new monsters for {}! Monster ID = {}", root_id.clone(), token_id_with_timestamp.clone());
        ...
        log!("Success! Transfering new monster for {}! Monster ID = {}", receiver_id.clone(), token_id_with_timestamp.clone());
