...
const ownerAccount = process.env.MASTER_ACCOUNT || fs.readFileSync('./neardev/dev-account').toString();

// Test admin management
console.log("..................................");
console.log("Administrators set management...");
sh.exec(`near view ${contractName} is_admin '{"admin_id": "${ownerAccount}"}'`); // should equal true
sh.exec(`near view ${contractName} is_admin '{"admin_id": "ilerik.testnet"}'`); // should equal false
sh.exec(`near call ${contractName} approve_admin '{"admin_id": "ilerik.testnet"}' --accountId ${ownerAccount} --gas 300000000000000`);
sh.exec(`near view ${contractName} is_admin '{"admin_id": "ilerik.testnet"}'`); // should equal true
sh.exec(`near call ${contractName} revoke_admin '{"admin_id": "ilerik.testnet"}' --accountId ${ownerAccount} --gas 300000000000000`);
sh.exec(`near view ${contractName} is_admin '{"admin_id": "ilerik.testnet"}'`); // should equal false
console.log("..................................");
...
sh.exec(`near view ${contractName} get_event_stats`);
sh.exec(`near call ${contractName} checkin '{"username": "ilerik.testnet", "request": "Congrats! Now you know more about Web3" }' --accountId ${contractName} --depositYocto 8350000000000000000000 --gas 300000000000000`);
sh.exec(`near call ${contractName} checkin '{"username": "sergantche.testnet", "request": "You have registered in the NEAR community" }' --accountId ${contractName} --gas 300000000000000`);
sh.exec(`near view ${contractName} get_event_stats`);
